...
SET SQL_SAFE_UPDATES=0;
update DMHP.ReportData SET FacilityTypeId=1,FacilityType='PHC' where FacilityId in (select distinct FacilityId from tbl_facility where FacilityTypeId=1);
update DMHP.ReportData SET FacilityTypeId=2,FacilityType='CHC' where FacilityId in (select distinct FacilityId from tbl_facility where FacilityTypeId=2);
update DMHP.ReportData SET FacilityTypeId=3,FacilityType='Taluka Hospital' where FacilityId in (select distinct FacilityId from tbl_facility where FacilityTypeId=3);
update DMHP.ReportData SET FacilityTypeId=4,FacilityType='MnC' where FacilityId is NULL;
SET SQL_SAFE_UPDATES=1;
